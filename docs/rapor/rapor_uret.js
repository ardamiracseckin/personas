...
const govde = [
  ...
  table(
    ...
    [
      ...
      ["Test seti: cevaplanabilir, cevaplanamaz ve uç durum soruları", "eval/questions.json — 44 soru, beş kategori (yazım hatalı sorular dâhil)"],
      ["Yanıt süresi ~1–3 saniye", "Kısmen: p50 4,5 sn, p95 10,3 sn (üretim sınırı ayarlandıktan sonra; öncesinde p95 48 sn idi)"],
  ...
  table(
    ...
    [
      ...
      ["Arayüz", "CLI + Streamlit", "CLI hata ayıklama ve ölçüm için, Streamlit demo için"],
  ...
  table(
    ...
    [
      ...
      ["Ortalama yanıt süresi", "4,60 sn"],
      ["p50 / p95 yanıt süresi", "4,50 sn / 10,27 sn"],
      ["Uç durumlarda çökme", "0"],
      ["Birim, HTTP ve regresyon testleri", "167 test, tamamı geçiyor"],
    ],
    [4680, 4680],
  ),
  spacer(),
  p("Çekimserlik iki katmanda çalışır ve ölçüm, birinci katmanın belirleyici olduğunu göstermiştir. Cevaplanamaz altı sorunun beşinde benzerlik eşiği hiç parça bırakmamış, asistan dil modelini hiç çağırmadan ortalama 0,02 saniyede bilgisi olmadığını söylemiştir."),
  p("Eşiği aşan tek soruda (\"Fotoğrafta diyafram değeri neyi etkiler?\") modeller ayrışmıştır: phi-4-mini bağlamdaki alakasız macOS parçasını kullanmamış, ancak istemdeki \"yalnızca bağlamı kullan\" kuralını da çiğneyerek kendi genel bilgisinden doğru bir fotoğrafçılık cevabı vermiştir. Aynı soruda qwen2.5-1.5b çekimser kalmıştır. Bu, seçilen modelin bilinen ödünüdür: daha güçlü model, bağlam dışına çıkma eğilimini de beraberinde getirmektedir. Bilgi tabanının kapsamı dışındaki sorularda cevabın kaynağı gösterilmediği için kullanıcı bu durumu ayırt edebilir."),
