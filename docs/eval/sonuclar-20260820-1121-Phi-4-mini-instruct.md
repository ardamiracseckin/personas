# Değerlendirme sonuçları — 20.08.2026 11:21
...
|---|---|---|---|---|---|---|---|
| C01 | cevaplanabilir | Ekran görüntüsünü belirli bir bölgeden n | Ekran görüntüsünü belirli bir bölgeden almak için Cmd + Shift + 4 tuşlarına basın, ardından seçilen bölgeyi se | macos-kisayollar.md | 7.31 | 2 |   |
| C02 | cevaplanabilir | Git'te son commit'i geri alıp değişiklik | Son commit'i geri alıp değişiklikleri koruduğunuzda, `git reset --soft HEAD~1` komutunu kullanın. Bu, son comm | git-notlari.md | 7.08 | 2 |   |
| C03 | cevaplanabilir | Yarım kalan değişiklikleri geçici olarak | `git stash` kullanın. | git-notlari.md | 7.61 | 2 |   |
| C04 | cevaplanabilir | Klasördeki gizli dosyaları da listeleyen | `ls -la` komutunu kullanarak klasörde gizli dosyaları da listeleyebilirsiniz. | terminal-komutlar.md, macos-kisayollar.md, vscode-notlari.md | 7.17 | 2 |   |
| C05 | cevaplanabilir | Bir dosyanın içinde metin aramak için ha | `grep "aranacak-kelime" dosya.txt` | terminal-komutlar.md | 6.58 | 2 |   |
| C06 | cevaplanabilir | Python'da sanal ortam nasıl oluşturulur  | Sanal ortam oluşturmak için `python3 -m venv .venv` çalıştırın. Etkinleştirmek için terminalde `.venv/bin/acti | python-notlari.md, vscode-notlari.md | 49.01 | 2 |   |
| C07 | cevaplanabilir | requirements.txt içindeki bağımlılıkları | `pip install -r requirements.txt` kullanın. | python-notlari.md | 4.31 | 2 |   |
| C08 | cevaplanabilir | SQLite'ta tablo zaten varsa hata almamak | `CREATE TABLE IF NOT EXISTS` | sqlite-notlari.md | 4.50 | 2 |   |
| C09 | cevaplanabilir | Embedding vektörleri SQLite'ta hangi biç | JSON metni olarak `TEXT` sütununda ya da ikili veri olarak `BLOB` sütununda saklanır. | sqlite-notlari.md, rag-kavramlari.md | 5.11 | 2 |   |
| C10 | cevaplanabilir | Foundry Local'de yüklü bir modeli bellek | `foundry model unload model-adi` komutunu kullanarak Foundry Local'de yüklü bir modeli bellekten boşaltabilirs | foundry-local-notlari.md | 48.18 | 2 |   |
| C11 | cevaplanabilir | Foundry Local servisinin adresi neden ko | Adres koda gömülmemelidir çünkü Foundry Local dinamik bir yerel port kullanır. | foundry-local-notlari.md | 5.32 | 2 |   |
| C12 | cevaplanabilir | RAG'in üç adımı nedir? | Retrieve, Augment, Generate | rag-kavramlari.md | 3.60 | 2 |   |
| C13 | cevaplanabilir | Benzerlik eşiği çok yüksek olursa ne olu | Bu konuda bilgim yok. | rag-kavramlari.md | 3.30 | 0 |   |
| C14 | cevaplanabilir | VS Code'da projenin tamamında nasıl aram | VS Code'da projenin tamamında arama yapmak için Cmd + Shift + F kullanın. | vscode-notlari.md, python-notlari.md | 5.03 | 2 |   |
...
| U02 | uc_durum | git | `git reset --soft HEAD~1` komutunu kullanarak son commit'i geri alıp değişiklikleri çalışma alanında koruyabil | git-notlari.md | 4.96 | — |   |
| U03 | uc_durum | nasıl? | Verilen bağlamda metni vektöre çevirin. Vektörler embeddisi metinler arasındaki anlamlılıkla karşılaştırılabil | rag-kavramlari.md | 44.13 | — |   |
| U04 | uc_durum | Bilgisayarımda hem git hem python hem sq | Sorunuzun bağlamında hem git hem de python hem sqlite hem de Foundry Local kullanıyorsunuz. Bu araçları öğrenm | vscode-notlari.md, python-notlari.md | 46.82 | — |   |
