# Değerlendirme sonuçları — 25.08.2026 14:08
...
| Sadakat (ortalama) | %67 — 16 cevapta ölçüldü |
...
|---|---|---|---|---|---|---|---|---|
| C01 | cevaplanabilir | Ekran görüntüsünü belirli bir bölgeden n | Ekran görüntüsünü belirli bir bölgeden almak için Cmd + Shift + 4 tuşlarına basın, ardından seçilen bölgeyi se | macos-kisayollar.md | 4.63 | 2 | 0.78 |   |
| C02 | cevaplanabilir | Git'te son commit'i geri alıp değişiklik | Son commit'i geri alıp değişiklikleri koruduğunuzda, `git reset --soft HEAD~1` komutunu kullanın. Bu, son comm | git-notlari.md | 5.33 | 2 | 0.60 |   |
| C03 | cevaplanabilir | Yarım kalan değişiklikleri geçici olarak | `git stash` kullanın. | git-notlari.md | 2.21 | 2 | 0.67 |   |
| C04 | cevaplanabilir | Klasördeki gizli dosyaları da listeleyen | `ls -la` komutunu kullanın. | terminal-komutlar.md, macos-kisayollar.md, vscode-notlari.md | 3.15 | 2 | 0.50 |   |
| C05 | cevaplanabilir | Bir dosyanın içinde metin aramak için ha | `grep "aranacak-kelime" dosya.txt` | terminal-komutlar.md | 2.96 | 2 | 1.00 |   |
| C06 | cevaplanabilir | Python'da sanal ortam nasıl oluşturulur  | Sanal ortam oluşturmak için `python3 -m venv .venv` çalıştırın. Etkinleştirmek için terminalinizi açın, `.venv | python-notlari.md, vscode-notlari.md | 6.65 | 2 | 0.65 |   |
| C07 | cevaplanabilir | requirements.txt içindeki bağımlılıkları | `pip install -r requirements.txt` kullanın. | python-notlari.md | 1.69 | 2 | 0.67 |   |
| C08 | cevaplanabilir | SQLite'ta tablo zaten varsa hata almamak | `CREATE TABLE IF NOT EXISTS` | sqlite-notlari.md | 3.03 | 2 | 1.00 |   |
| C09 | cevaplanabilir | Embedding vektörleri SQLite'ta hangi biç | JSON metni olarak `TEXT` sütununda veya ikili veri olarak `BLOB` sütununda saklanır. | sqlite-notlari.md, rag-kavramlari.md | 3.76 | 2 | 1.00 |   |
| C10 | cevaplanabilir | Foundry Local'de yüklü bir modeli bellek | `foundry model unload model-adi` komutunu kullanarak Foundry Local'de yüklü bir modeli bellekten boşaltabilirs | foundry-local-notlari.md | 6.26 | 2 | 0.58 |   |
| C11 | cevaplanabilir | Foundry Local servisinin adresi neden ko | Adres koda gömülmemelidir çünkü Foundry Local dinamik bir yerel port kullanır. | foundry-local-notlari.md | 3.64 | 2 | 0.86 |   |
| C12 | cevaplanabilir | RAG'in üç adımı nedir? | Retrieve, Augment, Generate. | rag-kavramlari.md | 2.08 | 2 | 1.00 |   |
| C13 | cevaplanabilir | Benzerlik eşiği çok yüksek olursa ne olu | Çok yüksek olursa 'Bilmiyorum' diyor. | rag-kavramlari.md | 2.64 | 2 | — |   |
| C14 | cevaplanabilir | VS Code'da projenin tamamında nasıl aram | VS Code'da projenin tamamında arama yapmak için Cmd + Shift + F kullanın. | vscode-notlari.md, python-notlari.md | 3.54 | 2 | 0.50 |   |
...
| B04 | cevaplanamaz | Excel'de düşeyara formülü nasıl yazılır? | Belgelerimde bu konuda bilgi yok. | — | 0.08 | — | — |   |
| B05 | cevaplanamaz | Ev kredisi faiz oranları nedir? | Belgelerimde bu konuda bilgi yok. | — | 0.09 | — | — |   |
| B06 | cevaplanamaz | Fotoğrafta diyafram değeri neyi etkiler? | Belgelerimde bu konuda bilgi yok. | — | 0.08 | — | — |   |
| U01 | uc_durum | (boş) | Belgelerimde bu konuda bilgi yok. | — | 0.01 | — | — |   |
| U02 | uc_durum | git | Sorunun git ile ilgili olduğunu anladım. Lütfen daha fazla ayrıntı veya belirli bir git komutuna ihtiyacınız v | git-notlari.md | 3.93 | — | 0.08 |   |
| U03 | uc_durum | nasıl? | Verilen bağlamda metni vektöre çevirin:  Sorun: Nasıl yapılır?  Vektörler: [how_to_do_it]   Verilen bağlamda e | rag-kavramlari.md | 3.83 | — | 0.50 |   |
| U04 | uc_durum | Bilgisayarımda hem git hem python hem sq | Sorunuzun bağlamında hem git hem de python hem sqlite hem de Foundry Local kullanıyorsunuz. Bu araçları öğrenm | vscode-notlari.md, python-notlari.md | 19.81 | — | 0.29 |   |
...
Eşiğin altında kalan 6 cevap:

| ID | Soru | Sadakat | Bağlamda karşılığı olmayan kelimeler |
|---|---|---|---|
| C04 | Klasördeki gizli dosyaları da listeleyen | 0.50 | komutunu, kullanin |
| C10 | Foundry Local'de yüklü bir modeli bellek | 0.58 | kullanarak, bosaltabilirsiniz, kullanmaniz, gerekecektir, alanini, gerekli, on, kosuludur |
| C14 | VS Code'da projenin tamamında nasıl aram | 0.50 | yapmak, kullanin |
| U02 | git | 0.08 | sorunun, ilgili, oldugunu, anladim, lutfen, fazla, ayrinti, belirli |
| U03 | nasıl? | 0.50 | verilen, baglamda, yapilir, how, to, do, it |
| U04 | Bilgisayarımda hem git hem python hem sq | 0.29 | sorunuzun, baglaminda, kullaniyorsunuz, araclari, ogrenmek, sirayi, takip, edebilirsiniz |
