# Değerlendirme sonuçları — 21.08.2026 19:20
...
|---|---|---|---|---|---|---|---|
| C01 | cevaplanabilir | Ekran görüntüsünü belirli bir bölgeden n | Ekran görüntüsünü belirli bir bölgeden almak için Cmd + Shift + 4 tuşlarına basın, ardından seçilen bölgeyi se | macos-kisayollar.md | 7.28 | 2 |   |
| C02 | cevaplanabilir | Git'te son commit'i geri alıp değişiklik | `git reset --soft HEAD~1` kullanın. Bu, son commit'i geri alır ama dosyalara dokunmaz. | git-notlari.md | 5.55 | 2 |   |
| C03 | cevaplanabilir | Yarım kalan değişiklikleri geçici olarak | `git stash` kullanın. | git-notlari.md | 4.64 | 2 |   |
| C04 | cevaplanabilir | Klasördeki gizli dosyaları da listeleyen | `ls -la` komutunu kullanın. | terminal-komutlar.md, macos-kisayollar.md, vscode-notlari.md | 4.63 | 2 |   |
| C05 | cevaplanabilir | Bir dosyanın içinde metin aramak için ha | `grep "aranacak-kelime" dosya.txt` | terminal-komutlar.md | 4.15 | 2 |   |
| C06 | cevaplanabilir | Python'da sanal ortam nasıl oluşturulur  | Sanal ortam oluşturmak için `python3 -m venv .venv` çalıştırın. Etkinleştirmek için terminalinizi açın, `.venv | python-notlari.md, vscode-notlari.md | 7.44 | 2 |   |
| C07 | cevaplanabilir | requirements.txt içindeki bağımlılıkları | `pip install -r requirements.txt` kullanın. | python-notlari.md | 3.09 | 2 |   |
| C08 | cevaplanabilir | SQLite'ta tablo zaten varsa hata almamak | `CREATE TABLE IF NOT EXISTS` | sqlite-notlari.md | 6.20 | 2 |   |
| C09 | cevaplanabilir | Embedding vektörleri SQLite'ta hangi biç | JSON metni olarak `TEXT` sütununda veya ikili veri olarak `BLOB` sütununda saklanır. | sqlite-notlari.md, rag-kavramlari.md | 6.75 | 2 |   |
| C10 | cevaplanabilir | Foundry Local'de yüklü bir modeli bellek | `foundry model unload model-adi` komutunu kullanarak Foundry Local'de yüklü bir modeli bellekten boşaltabilirs | foundry-local-notlari.md | 8.68 | 2 |   |
| C11 | cevaplanabilir | Foundry Local servisinin adresi neden ko | Adres koda gömülmemelidir çünkü Foundry Local dinamik bir yerel port kullanır. | foundry-local-notlari.md | 6.45 | 2 |   |
| C12 | cevaplanabilir | RAG'in üç adımı nedir? | Retrieve, Augment, Generate. | rag-kavramlari.md | 4.00 | 2 |   |
| C13 | cevaplanabilir | Benzerlik eşiği çok yüksek olursa ne olu | Çok yüksek olursa 'Bilmiyorum' diyor. | rag-kavramlari.md | 3.74 | 2 |   |
| C14 | cevaplanabilir | VS Code'da projenin tamamında nasıl aram | VS Code'da projenin tamamında arama yapmak için Cmd + Shift + F kullanın. | vscode-notlari.md, python-notlari.md | 5.66 | 2 |   |
| B01 | cevaplanamaz | Docker imajı nasıl oluşturulur? | Belgelerimde bu konuda bilgi yok. | — | 0.12 | — |   |
...
| U02 | uc_durum | git | Git stazhi kullanarak geçici olarak saklamış olduğunuz değişiklikleri geri getirin. | git-notlari.md | 5.78 | — |   |
| U03 | uc_durum | nasıl? | Verilen bağlamda metni vektöre çevirin:  Sorun: Nasıl yapılır?  Vektörler: [how_to_do_it]   Verilen bağlamda e | rag-kavramlari.md | 5.31 | — |   |
| U04 | uc_durum | Bilgisayarımda hem git hem python hem sq | Sorunuzun bağlamında hem git hem de python hem sqlite hem de Foundry'nun yerleşik sanal ortamını kullanıyorsun | vscode-notlari.md, python-notlari.md | 21.84 | — |   |
